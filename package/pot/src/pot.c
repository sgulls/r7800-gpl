...
//        char *net_conn;
	...
	for (;;) {
		...
		potval[sizeof(potval) - 1] = '\0';
		DNI_sprintf_s(potval + DNI_strnlen_s(potval, sizeof(potval)), sizeof(potval) - DNI_strnlen_s(potval, sizeof(potval)), "MAC%02x-%02x-%02x-%02x-%02x-%02x", mac[0], mac[1], mac[2], mac[3], mac[4], mac[5]);
		potval[sizeof(potval) - 1] = '\0';
