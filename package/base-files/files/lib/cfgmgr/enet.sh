...
{
	local i mask=$(($1 & 0xf))
	local ports1="6" ports2="0 5" ports3="0t 5t"
	...
	for i in 1 2 3 4; do
		...
		if [ "$iptv_vlan_enable" = "1" ]; then
			[ $(( $(i_mask $i) & $mask )) -eq 0 ] || ports3="$ports3 ${i}t"
		fi
	done
	...
	[ "$iptv_vlan_enable" = "1" ] && sw_printconf_add_vlan "switch0" "$iptv_vlan" "$iptv_vlan" "$ports3" >> $swconf
