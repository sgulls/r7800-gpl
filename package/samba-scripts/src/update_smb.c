...
static char *str_replace(char *org)
{
	char c, *p;
	static char buf[128];
...
static void add_smbd_share_info(FILE *fp, char *displayname, char *reader, char *writer, char *path, int share_floder_num)
{
	...
	if((strcmp(readycloud_enable_info,"1") != 0) || (readycloud_access_info[0] == '\0')){
		snprintf(name, sizeof(name),SHARE_AUTH_INFO"%d", share_floder_num);
...
static char * get_usb_serial_num(char *part_name)
{
	...
	char disk_name[8], path[64], line[128], cmd[256], disk_name_replace[8] = {0};
	char *bus_num = NULL;;
	char *p;
	int j = 0;
	FILE *fp;
	...
	disk_name[3] = '\0';
	strncpy(disk_name_replace, str_replace(disk_name), sizeof(disk_name_replace)-1);
	snprintf(cmd, sizeof(cmd),"/bin/ls /sys/block/%s/device/scsi_device/ > /tmp/%s_info_tmp", disk_name_replace, disk_name_replace);
	system(cmd);
...
#define SD_CARD_UUID_FILE "/tmp/sd_card_uuid"
	...
	FILE *fp;
        buf[0] = '\0';
     	...
     	snprintf(cmd, sizeof(cmd), "/usr/sbin/vol_id -u /dev/%s > "SD_CARD_UUID_FILE, str_replace(part_name));
       	system(cmd);
...
#define VOLUME_ID_FILE "/tmp/vol_id_1"
	...
	int i;
	FILE *fp;
	char *buf, disknum[4], diskname[4], capacity[32], cmd[256], diskname_replace[4] = {0};
        buf = disk->vol_name;
        buf[0] = '\0';
	
	snprintf(cmd, sizeof(cmd), "/usr/sbin/vol_id -L /dev/%s > " VOLUME_ID_FILE, str_replace(part_name));
       	system(cmd);
	...
	if (buf[0] == '\0'){
		...
		disknum[1] = '\0';
		strncpy(diskname_replace, str_replace(diskname), sizeof(diskname)-1);
...
#define DISK_LOOP_PARTITION "/tmp/disk_loop_partition"
       ...
       diskname[3] = '\0';
       snprintf(cmd, sizeof(cmd), "/usr/sbin/parted -s /dev/%s print | grep \"Partition Table\" | awk '{print $3}' > " DISK_LOOP_PARTITION, str_replace(diskname));
       system(cmd);
...
{
	FILE *fp;
	char diskname[4], cmd[128], result[8], diskname_replace[4] = {0};
	...
	strncpy(diskname_replace, str_replace(diskname), sizeof(diskname_replace)-1);
	snprintf(cmd, sizeof(cmd), "/usr/sbin/parted -s /dev/%s print noshare | grep %s", diskname_replace, str_replace(dev));
	fp = popen(cmd, "r");
	if (!fp) {
		perror("popen");
		return 0;
...
static int check_approved_disk(struct disk_partition_info *diskinfo)
{
	...
	i = 1;
        for (; ; i++) {
                ...
                len = snprintf(value, sizeof(value),"%s", config_get(approved_info));
                if (len < 1)
                        break;
		p = value + len;
		for( ; *p != '*'; p--);
