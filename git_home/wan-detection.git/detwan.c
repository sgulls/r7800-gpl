...
static int PPTP_Detection(void)
{
	...
	char cmd[128];
	snprintf(cmd, sizeof(cmd), "/sbin/route add -net %s netmask 255.255.255.255 %s", DEF_PPTP_SERVER, wan_if_name);
	system(str_replace(cmd));
	snprintf(cmd, sizeof(cmd), "/usr/sbin/iptables -t mangle -I PREROUTING -i %s -s %s -d %s -p tcp --sport %d -j ACCEPT", wan_if_name, DEF_PPTP_SERVER, DEF_PPTP_IPADDR, PPTP_PORT);
	system(str_replace(cmd));
...
static int mac_spoofing(void)
{
	...
	snprintf(cmd, sizeof(cmd), "/sbin/ifconfig %s hw ether %s", wan_if_name, mac);
	system(str_replace(cmd));
...
void Internet_Detection(void)
{
	...
	syslog(6, "[DETWAN] *Internal* from_ip: %s ,wan_if_name: %s ,dutMac: %s ,pc_mac: %s",
		strReplace(inet_ntoa(from_ip)), strReplace(wan_if_name), strReplace(config_get("wan_factory_mac")), strReplace(pc_mac));
