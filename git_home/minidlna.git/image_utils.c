...
image_get_jpeg_date_xmp(const char * path, char ** date)
{
	...
	while( !feof(img) )
	{
		...
		if( buf[0] == 0xE1 ) // APP1 marker
		{
			...
			length = (unsigned int)offset+1;
			newdata = realloc(data, length);
			if( !newdata )
				break;
			data = newdata;
...
image_new(int32_t width, int32_t height)
{
	...
	length = (unsigned int)(width * height * sizeof(pix));
	if((vimage->buf = (pix *)malloc(length)) == NULL)
	{
		DPRINTF(E_WARN, L_METADATA, "malloc failed\n");
		free(vimage);
		return NULL;
...
image_save_to_jpeg_buf(image_s * pimage, int * size)
{
	...
	length = (unsigned int)row_stride;
