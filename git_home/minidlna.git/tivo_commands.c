...
ProcessTiVoCommand(struct upnphttp *h, const char *orig_path)
{
	...
	while( item != NULL )
	{
		...
		if( strcasecmp(flag_p, "Command") == 0 )
		{
			command = val;
		}
		else if( strcasecmp(flag_p, "Container") == 0 )
		{
			container = val;
		}
		else if( strcasecmp(flag_p, "ItemStart") == 0 )
		{
			itemStart = atoi(val);
		}
		else if( strcasecmp(flag_p, "ItemCount") == 0 )
		{
			itemCount = atoi(val);
		}
		else if( strcasecmp(flag_p, "AnchorItem") == 0 )
		{
			anchorItem = basename(val);
		}
		else if( strcasecmp(flag_p, "AnchorOffset") == 0 )
		{
			anchorOffset = atoi(val);
		}
		else if( strcasecmp(flag_p, "Recurse") == 0 )
		{
			recurse = strcasecmp("yes", val) == 0 ? 1 : 0;
		}
		else if( strcasecmp(flag_p, "SortOrder") == 0 )
		{
			sortOrder = val;
		}
		else if( strcasecmp(flag_p, "Filter") == 0 )
		{
			filter = val;
		}
		else if( strcasecmp(flag_p, "RandomSeed") == 0 )
		{
			randomSeed = strtoul(val, NULL, 10);
		}
		else if( strcasecmp(flag_p, "Url") == 0 )
		{
			if( val )
				detailItem = strtoll(basename(val), NULL, 10);
		}
		else if( strcasecmp(flag_p, "SourceFormat") == 0 )
		{
			sformat = val;
		}
		else if( strcasecmp(flag_p, "Format") == 0 || // Only send XML
		         strcasecmp(flag_p, "SerialNum") == 0 || // Unused for now
		         strcasecmp(flag_p, "DoGenres") == 0 ) // Not sure what this is, so ignore it
		{
			;;
		}
		else
