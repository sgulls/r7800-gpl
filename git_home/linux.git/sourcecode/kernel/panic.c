...
int save_kernel_msg_to_flash()
{
	...
	else
	{
		...
		redirect_console_oops_msg(dni_msg, PANIC_MSG_LEN);
		#if 1
		...
		char dni_msg_bak[PANIC_MSG_LEN + 1];
		char *msg_back=dni_msg;
			
		int i;
		for(i=0; i<PANIC_MSG_LEN ; i++)
		{
			if(strncmp(msg_back,"Unable to handle kernel NULL pointer",36) == 0)
			{
				msg_back=msg_back-1;
				strcpy(dni_msg,msg_back);
				break;
			}
			else
			{
		...
		//write a block full
		if(((count + 1)*128*1024) - offset < SKIP_LENGTH){
